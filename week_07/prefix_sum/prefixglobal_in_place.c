...
int main(int argc, char** argv) {
    ...
    double start = now();
    {
        ...
        cl_program program = cluBuildProgramFromFile(context, device_id, "prefixglobal_in_place.cl", NULL);
        ...
        prefixSum(context,command_queue,reduce,expand,work_group_size,devDataA,N);

        // Part 6: download result from device
        err = clEnqueueReadBuffer(command_queue, devDataA, CL_TRUE, 0, N * sizeof(int), S, 0, NULL, NULL);
...
void prefixSum(cl_context context, cl_command_queue queue, cl_kernel reduce, cl_kernel expand, size_t work_group_size, cl_mem data, size_t size) {
    ...
    clSetKernelArg(reduce, 1, sizeof(cl_mem), &devDataSum);
    clSetKernelArg(reduce, 2, 2 * work_group_size * sizeof(int), NULL);     // two times the work group size!
    clSetKernelArg(reduce, 3, sizeof(size_t), &size);    
    ...
    prefixSum(context,queue,reduce,expand,work_group_size,devDataSum,num_groups);
    
    // -- expansion step --
    
    // extend result
    clSetKernelArg(expand, 0, sizeof(cl_mem), &data);
    clSetKernelArg(expand, 1, sizeof(cl_mem), &devDataSum);
    ...
    // submit kernel
    CLU_ERRCHECK(clEnqueueNDRangeKernel(queue, expand, 1, NULL, &global_size, &work_group_size, 0, NULL, NULL), "Failed to enqueue expansion kernel");
